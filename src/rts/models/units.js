...
export const TOOL_BIT = { axe: 1, pick: 2, hammer: 4, basket: 8, wood: 16, sack: 32, food: 64 };
export const ANIM = {
...
function pivotsFor(model) {
  const p = new Array(18).fill(0).map(() => [0, 0, 0]);
  ...
  for (const t of [PART.AXE, PART.PICK, PART.HAMMER]) p[t] = p[PART.ARM_R];
  p[PART.BASKET] = p[PART.BODY]; p[PART.CARRY_WOOD] = p[PART.BODY]; p[PART.CARRY_SACK] = p[PART.BODY]; p[PART.CARRY_FOOD] = p[PART.BODY];
...
void main() {
  ...
  else if (part == 4 || part == 5 || part == 6 || part == 7) R = rZ(armRZ) * rX(armRA);
  ...
  bool upper = part == 0 || (part >= 3 && part <= 11);
  if (upper && !rider) {
    vec3 hp = uPivots[0];
    mat3 B = rX(bodyA);
    p = hp + B * (p - hp);
...
function unitMaterial(pivots, rider, shadow) {
  return new ShaderMaterial({
    ...
    uniforms: { uPivots: { value: pivots }, uRider: { value: rider ? 1 : 0 } },
...
export class UnitBatch {
  constructor(pipeline, model, capacity = 256) {
    ...
    this.mesh = new InstancedMesh(geo, unitMaterial(pivots, rider, false), capacity);
    this.mesh.instanceMatrix.setUsage(DynamicDrawUsage);
    this.mesh.count = 0;
    this.mesh.frustumCulled = false;
    pipeline.scene.add(this.mesh);
    pipeline.addShadowCaster(this.mesh, unitMaterial(pivots, rider, true));
  }
